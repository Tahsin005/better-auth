...
export async function POST(request: Request) {
    ...
    if (decision.isDenied()) {
        if (decision.reason.isRateLimit()) {
            ...
            let message: string = "Invalid email address";
...
async function checkArcjet(request: Request) {
    ...
    if (request.url.endsWith("/auth/sign-up")) {
        if (body && typeof body === "object" && "email" in body && typeof body.email === "string") {
            ...
            // github or discord or other sign in methods
            return aj.withRule(detectBot(botSettings)).withRule(slidingWindow(restrictiveRateLimitSettings)).protect(request, {
                userIdOrIp
            })
        }
    }

    return aj.withRule(detectBot(botSettings)).withRule(slidingWindow(laxRateLimitSettings)).protect(request, {
        userIdOrIp
    });
}